...
enum ImageAttachmentEncoder {
    /// Downscales (only if larger than `maxDimension`) and JPEG-encodes image bytes so
    /// vision/edit requests keep source detail instead of being pre-degraded client-side.
    static func makeAttachment(from imageData: Data, maxDimension: CGFloat = 2688, quality: CGFloat = 0.92) -> ChatImageAttachment? {
        #if canImport(UIKit)
    ...
    #if canImport(UIKit)
    static func makeAttachment(from image: UIImage, maxDimension: CGFloat = 2688, quality: CGFloat = 0.92) -> ChatImageAttachment? {
