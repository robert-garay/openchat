...
struct MessageBubbleView: View {
    ...
    private var assistantContent: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                if !pendingMemoryProposals.isEmpty { memoryConfirmationCard } else if let memoryActionStatus, !memoryActionStatus.isEmpty { Text(memoryActionStatus).font(.caption).foregroundStyle(.secondary) }
    ...
    private var memoryConfirmationCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            Label("Save to memory?", systemImage: "brain.head.profile").font(.subheadline.weight(.semibold))
            ForEach(pendingMemoryProposals) { Text($0.content).font(.caption).foregroundStyle(.secondary) }
            HStack { Button("Save") { onConfirmMemoryProposals?() }.buttonStyle(.borderedProminent); Button("Discard") { onDismissMemoryProposals?() }.buttonStyle(.bordered) }
        }.padding(12).frame(maxWidth: .infinity, alignment: .leading).background(Color.secondary.opacity(0.12), in: RoundedRectangle(cornerRadius: 12))
    }
