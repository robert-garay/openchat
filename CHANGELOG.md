...
- Voice mode: tap the mic button in the composer for a live, interruptible spoken conversation (OpenAI Realtime API), with web search and skills available as voice-callable tools. Transcripts land in the same conversation as regular messages, so you can switch freely between typing and talking. Requires the chat's selected provider to be OpenAI with an API key configured. Pick which voice it speaks with in Settings → Tools → Voice Mode.
